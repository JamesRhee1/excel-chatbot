...
from core.workspace import LAST_RESULT_TABLE
...
_TOP_MAX_KEYWORDS = ("가장 높은", "최대", "제일 큰", "가장 큰", "최댓값", "가장 많은", "많이 남은", "제일 높은", "제일 많은")
...
_FILTER_NUM_NO_JOSA_PATTERN = re.compile(
    r"(.+?)\s*(\d+(?:\.\d+)?)\s*보다\s*(큰|높은|많은|작은|낮은|적은|초과|미만)"
)
_FILTER_THRESHOLD_PATTERN = re.compile(
    r"(.+?)(?:이|가)?\s*(\d+(?:\.\d+)?)\s*(이상|이하)"
)
_SHOW_ALL_PHRASES = ("다 보여줘", "전부 보여줘", "모두 보여줘", "전체 보여줘")
_COMPARISON_TOKENS = ("보다", "이상", "이하", "초과", "미만")
_NUMERIC_COMPARE_PATTERN = re.compile(
    r"\d+(?:\.\d+)?\s*보다|보다\s*\d+(?:\.\d+)?|\d+(?:\.\d+)?\s*(?:이상|이하|초과|미만)"
)
...
def _uses_last_result(msg: str) -> bool:
    return any(hint in msg for hint in _CONTEXT_SOURCE_HINTS)
...
def route_intent(user_message: str, profile: dict) -> dict | None:
    ...
    context_top = _try_context_top_n(msg, profile)
    if context_top:
        return context_top
    ...
    show_all_intent = _try_show_all(msg, profile)
    if show_all_intent:
        return show_all_intent
...
def _comparison_op(token: str) -> str:
    op_map = {
        "큰": ">",
        "높은": ">",
        "많은": ">",
...
def _has_comparison_tokens(msg: str) -> bool:
    if any(token in msg for token in _COMPARISON_TOKENS):
        return True
    return bool(_NUMERIC_COMPARE_PATTERN.search(msg))
...
def _try_filter(msg: str, profile: dict) -> dict | None:
    ...
    if match:
        ...
        op = _comparison_op(match.group(3))
        return _intent("dataframe", [{"type": "filter", "column": col, "op": op, "value": value}])

    match = _FILTER_NUM_NO_JOSA_PATTERN.search(msg)
    if match:
        col = _extract_column_hint(match.group(1), profile) or match.group(1).strip()
        value = float(match.group(2)) if "." in match.group(2) else int(match.group(2))
        op = _comparison_op(match.group(3))
    ...
    match = _FILTER_THRESHOLD_PATTERN.search(msg)
    if match:
        col = _extract_column_hint(match.group(1), profile) or match.group(1).strip()
        value = float(match.group(2)) if "." in match.group(2) else int(match.group(2))
        op = _comparison_op(match.group(3))
...
def _try_context_top_n(msg: str, profile: dict) -> dict | None:
    if not _uses_last_result(msg):
        return None
...
def _try_show_all(msg: str, profile: dict) -> dict | None:
    if not any(phrase in msg for phrase in _SHOW_ALL_PHRASES):
        return None
    if _has_comparison_tokens(msg):
        return None
    operations: list[dict] = []
    if _uses_row_types(profile):
        detail = (profile.get("summary_row_config") or {}).get("detail_row_type")
        if detail:
            operations.append({"type": "filter_row_type", "row_types": [detail]})
        else:
            operations.append({"type": "exclude_summary"})
    else:
        operations.append({"type": "exclude_summary"})
    return _intent("dataframe", operations)
